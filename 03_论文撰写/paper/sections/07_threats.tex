...
paper are therefore reported \emph{descriptively}: because records within a
design cell are not independent, they do not support a formal hypothesis-test
reading, and the recall difference in Boundary~2 in particular is not a test
outcome. The paper's conclusions do not rest on statistical significance.
...
\paragraph{Real systems, none vulnerable on the tested surface} The real-system study
...
on the endpoints and authorization transitions exercised, none presented an
object-level authorization vulnerability. The experiment
