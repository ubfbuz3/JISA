...
rate is \ResFpEthreeHits{}/\ResFpEthreeTotal{}. The
denominator is \ResFpEthreeTotal{}, so the rate is unstable by construction and
we do not attach an interval to it---nor a percentage, which would travel out of
...
drops to \ResBEThreevulnZerolistedFpPct{}\% but a false negative appears among
the five violating records (\ResBEThreevulnOnelistedFn{} of
\ResBEThreevulnOnelistedTp{}+\ResBEThreevulnOnelistedFn{}). Avoiding false proofs and keeping detection are
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.9\textwidth]{fig2_scenarios.pdf}
...
the framework's own log, while the state-calibrated v3 abstains exactly where its
surface is not readable. Both panels are recomputed from the same records as
...
a fixed enumeration, not a sample (Section~\ref{sec:stats}), so no inferential
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{fig4_methods.pdf}
\caption{The same comparison as Table~\ref{tab:ablation}, pooled over all
\ResSynRecords{} records (Section~\ref{sec:stratum} splits it by oracle
readability). The metamorphic step does not carry the detection load:
\texttt{direct\_observe} matches v3's false-proof count and recovers every
violation v3 misses, while v3's precision is bought with false negatives. Bars are
TP/FP/FN.}
\label{fig:methods}
\end{figure*}

Table~\ref{tab:ablation} concludes the ablation. \texttt{direct\_observe} has
...
\subsection{A published metric's resolution: what the reported specificity does not condition on}

The practical content of this finding is a diagnosis of a specific published
figure, not a general property of ratios. Under the input-level specificity
convention used by \mbox{MST-wi}~\cite{Chaleshtari2022Metamorphic}, the reported
specificity has the form
\[ S_{\text{input}} = 1 - \frac{\text{false alarms}}{\text{inputs}}, \]
and the figure of \ResLitReportedSpecPct{}\% is reported without an explicit
check that the reachability model whose readings drive adjudication coincides
with the true authorization state. Where the two diverge---the out-of-band case
of Finding~1---the metric records agreement with the tool's own crawler rather
than with entitlement, so a high value certifies the correspondence it silently
assumes. The predecessor itself concedes the role-to-resource mapping is ``not
always available'' and attributes its residual false alarms to crawler coverage
of ``all URLs for all users''~\cite{Mai2019Metamorphic}. The dilution below is
elementary arithmetic, offered only to make the second half of the
diagnosis---that the denominator cannot register a class-uniform failure---precise;
we do not present it as the contribution.
...
\ResSpecRowOneSpec{} (\ResSpecRowOneSpecPct{}\%)\footnote{This value is
illustrative of the denominator's resolution only. It rests on an assumed two
follow-up inputs per case, an approximation we cannot ground in the engine's
exact input count; what matters is the \emph{form} of the denominator, not the
value.}.
...
$S_{\mathrm{scenario}}$ is introduced only as an analytical construct for
characterising resolution; it is not proposed as a replacement evaluation
metric. The remark concerns metric resolution, not estimator consistency or
statistical power.

\begin{proposition}
\label{prop:resolution}
Fix $f \ge 1$ failed scenarios, each producing a
...
the $f$ failing scenarios remain failing.
\end{corollary}

\smallskip
The two halves of the finding are about the specific reported figure, not about
ratios in the abstract: its value records agreement with the tool's own crawler
where model and authorization state diverge, and its denominator cannot register
a class-uniform failure even when one occurs.
