...
We state the reason rather than present it as a discovered effect: its burden is
an empirical premise about the evaluated relations, not an algebraic identity.
...
(Section~\ref{sec:gogs}) and the seeded control (Section~\ref{sec:seeded}) complete the
...
Proposition~\ref{lem:oracle-suff} for the evaluated authorization proposition, so the
