...
\subsection{Boundary 2 (RQ2): with a sound, readable oracle, the metamorphic step adds no adjudication information}
\label{sec:boundary2}
...
The lemma is a requirement on \emph{claims}, not a verdict on metamorphic
...
false by construction (Boundary~1). Source inspection confirms the two relations
...
\subsection{Boundary 3 (RQ3): the reported specificity is not conditioned on a reachability--authorization check, and uses a low-resolution denominator}

\emph{The reported \ResLitReportedSpecPct{}\% is a statement about a corpus of
generated inputs; it is not conditioned on an explicit validation that the
reachability model coincides with the true authorization state.} The omission:
